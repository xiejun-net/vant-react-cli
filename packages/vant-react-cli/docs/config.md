...
Custom [vite config](https://vitejs.dev/config/), requires `vant-react-cli>= 4.0.0`.
...
If you need to configure some vite plugins, please create a `vite.config.ts` file in the same directory of `vant.config.mjs`, in which you can add any vite configuration (this feature requires vant-react-cli 5.1.0).
